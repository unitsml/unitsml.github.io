...
### Shipbuilding and maritime classification

The [Open Class 3D Exchange (OCX)](https://3docx.org) standard encodes structural and geometric ship data in XML for model-based class approval. OCX uses UnitsML to represent physical quantities such as lengths, masses, and centres of gravity, so that shipyards and classification societies can exchange measurement data without ambiguity.

### Industrial automation and product data dictionaries

UnitsML underpins unit representation in ECLASS, which provides semantic identifiers used by IEC/TS 62720 and the [IEC Common Data Dictionary (CDD)](https://cdd.iec.ch). This harmonization lets industrial automation systems reference units through stable, machine-readable IRDIs while remaining aligned with the SI.
